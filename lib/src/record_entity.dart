...
class RecordEntity {
  ...
  final DateFormat _formatter = DateFormat('d. M. yyyy');
  ...
  String toString() {
    ...
    final bool planned = date?.isAfter(DateTime.now()) ?? false;

    // link or simple text
    if (titleLink != null) {
      titleElement =
          '''<a class="record__title link" href="$titleLink" target="_blank">$title</a>
          ''';
    } else {
      titleElement =
          '''<span class="record__title ${planned ? 'record__title--todo' : ''}">$title</span>''';
    }
